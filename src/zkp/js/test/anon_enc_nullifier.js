...
const { Poseidon, newSalt, poseidonDecrypt } = require('../index.ts');

const SMT_HEIGHT = 64;
const poseidonHash = Poseidon.poseidon4;
const poseidonHash3 = Poseidon.poseidon3;

describe('main circuit tests for Zeto fungible tokens with encryption and anonymity using nullifiers', () => {
  ...
  before(async function () {
    ...
    circuit = await wasm_tester(join(__dirname, '../../circuits/anon_enc_nullifier.circom'));
  ...
  it('should fail to generate a witness because mass conservation is not obeyed', async () => {
    ...
    expect(err).to.match(/Error in template Zeto_258 line: 91/);
  });
});
